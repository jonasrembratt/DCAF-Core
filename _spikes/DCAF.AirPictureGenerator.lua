...
local kvIntensityMedium = KeyValue:New(3, Altitude.Medium)
local DCAF_IntensityValues = {
    VeryLow = KeyValue:New(1, Altitude.VeryLow),
    Low = KeyValue:New(2, Altitude.Low),
    Medium = kvIntensityMedium,
    High = KeyValue:New(4, Altitude.High),
    VeryHigh = KeyValue:New(5, Altitude.VeryHigh)
}
...
local kvAltitudeVeryHigh = KeyValue:New(Altitude.VeryHigh, 35000)
local DCAF_BVR_Altitudes = {
    kvAltitudePopup,
    KeyValue:New(Altitude.Low, 8000),
    KeyValue:New(Altitude.Medium, 15000),
    KeyValue:New(Altitude.High, 25000),
    kvAltitudeVeryHigh
}
...
local function formStack(generator, info, group, groupSize, skill, groupCount)
    ...
    local minAltitude = generator.Settings.Altitude.Min.Value
    local maxAltitude = generator.Settings.Altitude.Max.Value
...
local function setIntensity(generator, groups, info)
    ...
    if currentIntensity.Value == Altitude.VeryLow then
        restoreIntensityInterval = restoreIntensityInterval*4
    elseif currentIntensity.Value == Altitude.Low then
        restoreIntensityInterval = restoreIntensityInterval*3
    elseif currentIntensity.Value == Altitude.Medium then
...
local function getRandomAltitude(generator, form)
    return math.random(generator.Settings.Altitude.Min.Value, generator.Settings.Altitude.Max.Value)
end
...
local function getIntensityValues(generator)
    ...
    maxGroupSize = 4
    if intensity == Altitude.VeryLow then
        maxGroupSize = 2
        maxGroupCount = 2
        generateInterval = Minutes(3)
    elseif intensity == Altitude.Low then
        maxGroupSize = 3
        maxGroupCount = 3
        generateInterval = Minutes(2.5)
    elseif intensity == Altitude.Medium then
        maxGroupCount = 4
        generateInterval = 90
    elseif intensity == Altitude.High then
        maxGroupCount = 5
    else
        maxGroupCount = 7
    end
